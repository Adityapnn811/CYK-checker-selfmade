...
def CYK(word, Rules):
    ...
    #     print(eachrow)
    # print("n-1 = ",n-1)
    arr = [[set() for j in range(len(TabelCYK[0]))] for i in range(len(TabelCYK[0]))]
    for i in range(len(TabelCYK[0])):
        for j in range(i, len(TabelCYK[0])):
            k = len(TabelCYK[0]) - 1
            arr[k - i][j - i] = TabelCYK[i][j]
    arr2 = [[set() for j in range(len(TabelCYK[0]))] for i in range(len(TabelCYK[0]))]
    for i in range(len(arr[0])):
        for j in range(len(arr[0])):
            k = len(TabelCYK[0]) - 1 - i
            l = len(TabelCYK[0]) - 1
            arr2[l - j][i] = arr[k][j]
    for row in arr2:
        print(row)

print("Jangan lupa update aturan cfg dalam bentuk cnf di cnf.txt!!!")
string_cek = input("Masukkan string yang mau dicek: ")
CYK(string_cek, CNFtoCNFdict())